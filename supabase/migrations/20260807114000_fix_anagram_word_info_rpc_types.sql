-- dictionary source columns use varchar in production while the RPC contract
-- intentionally exposes text. Cast the public boundary explicitly.
...
begin
  ...
  select
    cards.normalized_word::text,
    coalesce(cards.dictionary_lemma, cards.norm_lemma, lower(cards.normalized_word))::text,
    (case when cards.entry_key is not null then 'verbo' else cards.sense_part_of_speech end)::text,
    coalesce(cards.relation_type, 'base')::text,
    (case when cards.entry_key is not null then cards.prime_sense else cards.sense_definition end)::text,
    cards.resolved_entry_key is not null,
    cards.entry_key is not null,
    cards.entry_key::numeric,
    cards.norm_lemma::text,
    cards.prime_sense::text,
    cards.prime_type::text,
    cards.regularity::text,
    cards.participle_masculine::text,
    cards.has_participle_masculine,
    cards.participle_masculine_plural::text,
    cards.has_participle_masculine_plural,
    cards.participle_feminine::text,
    cards.has_participle_feminine,
    cards.prnl_end::text,
    cards.voseo_imperative_plural::text,
