...
serve(async (req) => {
  ...
  try {
    ...
    const setupSQL = `
      ...
      -- Insertar patrón inicial de prueba
    ...
    // Execute setup using direct SQL (this is a one-time setup)
    console.log('Setting up training tables...');
    
    // Here we would execute the SQL - for now we return the SQL for manual execution
    return new Response(
      JSON.stringify({ 
        success: true,
        message: 'Training tables schema ready',
        sql: setupSQL,
        tables_created: [
          'training_patterns',
          'training_rules', 
